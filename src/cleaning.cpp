...
namespace Sim {
Cleaning::Cleaning(ros::NodeHandle& nh) : ComponentVirtualBase(nh, UPDATE_RATE), m_nh{nh}
{
    ...
    simGetShapeColor(m_robotBaseHandle, nullptr, sim_colorcomponent_ambient_diffuse,
                     m_colorYellow.data());
    m_cleaningServer = m_nh.advertiseService("cleaning_command", &Cleaning::CleaningService, this);
    m_tankRinseServer = m_nh.advertiseService("tank_rinse", &Cleaning::TankRinseService, this);
    m_liftDriveServer =
        m_nh.advertiseService("lift_drive_control", &Cleaning::LiftDriveService, this);
}
...
void Cleaning::TankRinse()
{
    if (m_clientCall)
    {
        ...
        if ((m_tickCount % 20) == 0)
        {
            simSetShapeColor(m_robotBaseHandle, nullptr, sim_colorcomponent_ambient_diffuse,
                             COLOR.data());
        }
...
void Cleaning::RunOnce(unsigned, unsigned)
{
    TankRinse();
}
const std::string_view Cleaning::Name() const
{
    return NAME;
}
bool Cleaning::LiftDriveService(kira_msgs::LiftDriveControl::Request& req,
                                kira_msgs::LiftDriveControl::Response& res)
{
    if (req.drive == kira_msgs::LiftDriveControl::Request::LIFT_DRIVE_BROOM)
    {
        BroomDrive(req);
    }
    else if (req.drive == kira_msgs::LiftDriveControl::Request::LIFT_DRIVE_BRUSH)
    {
        BrushDrive(req);
    }
    else if (req.drive == kira_msgs::LiftDriveControl::Request::LIFT_DRIVE_SQUEEGEE)
    {
        SqueeGeeDrive(req);
    }
    else if (req.drive == kira_msgs::LiftDriveControl::Request::LIFT_DRIVE_ALL)
    {
        BroomDrive(req);
        BrushDrive(req);
        SqueeGeeDrive(req);
    }
    res.error_code = kira_msgs::LiftDriveControl::Response::LIFT_NO_ERROR;
    return true;
}

void Cleaning::BroomDrive(const kira_msgs::LiftDriveControl::Request& req)
{
    if (req.command == kira_msgs::LiftDriveControl::Request::LIFT_CMD_UP)

    {
        ROS_INFO("Broom is lifting up ");
    }
    else if (req.command == kira_msgs::LiftDriveControl::Request::LIFT_CMD_DOWN)
    {
        ROS_INFO("Broom is lifting down ");
    }
    else if (req.command == kira_msgs::LiftDriveControl::Request::LIFT_CMD_STOP)
    {
        ROS_INFO("Broom lifting is stopped ");
    }
}

void Cleaning::BrushDrive(const kira_msgs::LiftDriveControl::Request& req)
{
    if (req.command == kira_msgs::LiftDriveControl::Request::LIFT_CMD_UP)
    {
        ROS_INFO("Brush is lifting up ");
    }
    else if (req.command == kira_msgs::LiftDriveControl::Request::LIFT_CMD_DOWN)
    {
        ROS_INFO("Brush is lifting down ");
    }
    else if (req.command == kira_msgs::LiftDriveControl::Request::LIFT_CMD_STOP)
    {
        ROS_INFO("Brush lifting is stopped ");
    }
}

void Cleaning::SqueeGeeDrive(const kira_msgs::LiftDriveControl::Request& req)
{
    if (req.command == kira_msgs::LiftDriveControl::Request::LIFT_CMD_UP)
    {
        ROS_INFO("Squeegee is lifting up ");
    }
    else if (req.command == kira_msgs::LiftDriveControl::Request::LIFT_CMD_DOWN)
    {
        ROS_INFO("Squeegee is lifting down ");
    }
    else if (req.command == kira_msgs::LiftDriveControl::Request::LIFT_CMD_STOP)
    {
        ROS_INFO("Squeegee lifting is stopped ");
    }
}
...
void Cleaning::TurnOnBroom(uint broom)
{
    ...
    if (broom)
    { /* turn on broom motor only when user requested */

        m_shape.speedBroom = MAX_MOTOR_SPEED + VALUE_X * static_cast<float>(std::min(broom, 100u));
...
void Cleaning::TurnOnBrush(uint brush)
{
    ...
    if (brush)
    { /* turn on brush motor only when user requested */

        m_shape.speedBrush = MAX_MOTOR_SPEED + VALUE_X * static_cast<float>(std::min(brush, 100u));
...
{
    m_waterPolygonHandle =
        simCreatePureShape(sim_pure_primitive_none, FLAGS, SIZE.data(), MASS, nullptr);
...
void Cleaning::ColorWater(uint amount)
{
    ...
    simSetShapeColor(m_waterPolygonHandle, nullptr, sim_colorcomponent_transparency,
                     &m_shape.opacity);
}
...
void Cleaning::TransformPolygon(uint amount, float ratio)
{
    if (amount && ratio)
    {
        ...
        if (!(m_shape.previousAmount == amount) && !(m_shape.previousRatio == ratio))
        {
            ColorWater(amount);
...
void Cleaning::ColorPolygon(uint amount, float ratio)
{
    if (amount && ratio)
    {
        ...
        simSetShapeColor(m_waterPolygonHandle, nullptr, sim_colorcomponent_ambient_diffuse,
                         m_shape.color.data());
    }
    else if (amount)
    {
