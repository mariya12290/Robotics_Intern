...
private:
  ...
  bool Delay();
=======
 * @details light control module service API for allowing the client to call the  service according
 * to operating modes. Operating modes are autonomous mode(driving forward and backward, turning
 * left and right), localization, docking, emergency, standard mode, cleaning, obstacle detection,
 * Leds on and off.
 */
class LightControlModule : public ComponentVirtualBase
{
public:
    ...
    /** we can add as many colors as we want, implementation is flexible */
    std::unordered_map<std::string, int> COLOR_OPTION{{"red", 1},    {"green", 2}, {"blue", 3},
                                                      {"yellow", 4}, {"white", 5}, {"orange", 6}};
    ...
    /** @brief member function to process the client request and to respond with response after
    ...
    /** @brief load the position and orientation for created leds in coppeliasim from .yaml file */
    void LoadLedsDescription();
...
private:
    ...
    struct Animation
    {
        /** array of given leds */
        sim_vector_l leds{};
        /** given function either snap for blinking and color change and linear for pulsing */
        std::string function{};
        /** given offset to synchronization  */
        uint offset{};
        /** RGB values for given leds states/colors */
    ...
    /** a dictionary to hold created animation/client call objects */
    /** the reason why map is used instead of unordered_map with compromising efficiency, bcoz we
     * need to remove the leds from previous client call objects, if the led is present in new
     * client call */
    std::map<std::string, Animation> m_serviceCall{};
    int m_tickCount{};
    sim_vector_t m_size{}; /**Leds size in m */
    ...
    /** @brief instantiation of animation/client call object for each client request
     *  @param client request parameters
     * @return animation/client call object
     */
    Animation AnimationObject(const kira_msgs::LedAnimationParams& params);

    /** @brief creation of animation objects and also deletion of former animation objects if number
     * of Leds in object is zero
     *  @param request parameters
     */
    void CreateAnimationObjects(const kira_msgs::LedAnimationParams& params);

    /** @brief function which will be executed when the client request is snap or for
     * flashing/blinking
     *  @param created animation or client call object and LEDs handle in coppeliasim
     */
    void LedFlash(LightControlModule::Animation& animation, sim_vector_l ledsHandle);

    /** @brief function which will be executed when the client request is linear or for pulsing
     *  @param created animation or client call object and LEDs handle in coppeliasim
     */
    void LedPulse(LightControlModule::Animation& animation, sim_vector_l ledsHandle);

    /** @brief function which will be executed when the client request is snap or just for color
     * change
     *  @param created animation or client call object and LEDs handle in coppeliasim
     */
    void LedColorChange(const LightControlModule::Animation& animation, sim_vector_l ledsHandle);
