 ...
 * not?
=======
 * @brief This file checks is_docked or not state and docking is possible are not?
...
=======
 * The position and orientation of kira will be checked for each tick relative to docking station
 * plugin and color of docking station will be changed when kira is docked. When there is a docking
 * service call, pulsing of docking station color will be taken place.
 */
...
public:
     ...
     *  @param provided service call request and service call response
     */
    bool DockingService(kira_msgs::Docking::Request& req, kira_msgs::Docking::Response& res);
    ...
    /** @brief check orientation of kira along z axis relative to docking station plugin, i.e yaw
     */
    bool CheckOrientation();

    /** @brief Check the y position bound  of kira relative to docking station plugin
     */
    bool CheckIsInBound();

    /** @brief Change the color of docking station when docking service is called/kira is docked
     */
    void UpdateDockingColor();
     ...
     * @param vector of string prefix
     */
    void ComponentHandleExtraction(std::vector<std::string_view> const& componentPrefixName);

    /** @brief load and place the docking station at the gievn position and orientation into the
     * simulation scene
     * @param path to the docking station model, given position and orientation of docking station
     * relative to kira base ref.
     */
    int AddDockingStation(const std::string& dockingStationPath);
