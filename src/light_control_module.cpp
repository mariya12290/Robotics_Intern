...
LightControlModule::LightControlModule(ros::NodeHandle &nh)
  ...
  LoadLedsDescription();
  CreateAndPlaceLeds();
  m_ledAnimationServer = m_nh.advertiseService(
      "light_control", &LightControlModule::AnimationServiceCallBack, this);
}

const std::string_view LightControlModule::Name() const { return NAME; }
void LightControlModule::Shutdown() { m_ledAnimationServer.shutdown(); }

bool LightControlModule::AnimationServiceCallBack(
    kira_msgs::LedAnimation::Request &req,
...
void LightControlModule::CreateAnimationObjects(
    const kira_msgs::LedAnimationParams &params) {
  if (m_callCount == 1) {
    Animation animation{AnimationObject(params)};
    std::pair<std::string, Animation> callObject(
        "call" + std::to_string(m_callCount), animation);
    m_serviceCall.insert(callObject);
    return;
  } else { // remove leds from old client call object if it is present in new
           // client request call
...
LightControlModule::Animation LightControlModule::AnimationObject(
    const kira_msgs::LedAnimationParams &params) {
  Animation animation;

  // input data validation is not implemented
  /** for an example, if the length of the leds is greater than 6 or if the
   * specified leds number is greater than 6. */
  ...
  std::transform(color_transition.begin(), color_transition.end(),
                 color_transition.begin(), [](std::string str) { return str; });
  animation.function = params.function;

  /** we can use directly duration and offset in seconds or in milliseconds.
   * Since in simulation we consider time step, duration and offset are
   * converted into timestep */
...
std::vector<std::array<float, 3>> LightControlModule::ColorValueExtraction(
    std::vector<std::string> &colorTransition) {
  std::vector<std::array<float, 3>> color{};
  auto option = [&](std::string str) {
    return (COLOR_OPTION.find(str) != COLOR_OPTION.end()
                ? (COLOR_OPTION.find(str)->second)
                : 0);
  };
  for (auto i{0u}; i < colorTransition.size(); i++) {
    ...
    case 1:
      color.push_back(
          {1, 0, 0}); // In vector of arrays, we can not use emplace_back
      break;
    case 2:
    ...
    default:
      ...
      ROS_WARN("RGB value for given color is not defined/ state transition is "
               "not given ");
      break;
    }
  }
...
void LightControlModule::RunOnce(unsigned, unsigned) { LedAnimation(); }
...
/** new flashing/blinking client call will always synchronize with the previous
flashing/blinking client call */
bool LightControlModule::Delay() {
  /** it is also possible to synchronize with second color instead of first */
  return m_tickCount == 0;
}

void LightControlModule::LedFlash(LightControlModule::Animation &animation,
                                  sim_vector_l ledsHandle) {
  if (animation.offset && animation.function == "snap" &&
      animation.colorValue.size() == 2 && m_clientCall) {
  ...
  /**  increasing the duration count of previous blinking client calls is also
   * possible in oder to synchronize with the new blinking client call through
   * creating separate container for blinking client call objects and check the
   * size of the container for everytick, if size increased which means,
   * duration count of previous blinking client objects should be increased for
   * the synchronization, while increasing duration count we need to take offset
   * of new blinking client call into account */

  if (animation.colorValue.size() == 2 && m_clientCall &&
      animation.function == "snap") {
    auto result = static_cast<float>(animation.tick) /
                  static_cast<float>(animation.colorValue.size());
    if (animation.tickCount == animation.tick || animation.tickCount == 0) {
      animation.colorIndex1 = 0;
      animation.tickCount = 0;
      animation.halfCount = 0;

    } else if ((animation.halfCount % static_cast<int>(std::ceil(result))) ==
               0) {
    ...
    std::for_each(animation.leds.begin(), animation.leds.end(), [&](int value) {
      simSetShapeColor(ledsHandle[value - 1], nullptr,
...
void LightControlModule::LedColorChange(
    const LightControlModule::Animation &animation, sim_vector_l ledsHandle) {
  if (animation.colorValue.size() == 1 && m_clientCall &&
      animation.function == "snap") {
    {
      std::for_each(animation.leds.begin(), animation.leds.end(),
                    [&](int value) {
                      simSetShapeColor(ledsHandle[value - 1], nullptr,
                                       sim_colorcomponent_ambient_diffuse,
...
/** It is also possible to use the offset in plusing, which means to say wait
 * for certain offset and then start */
void LightControlModule::LedPulse(Animation &animation,
                                  sim_vector_l ledsHandle) {
  if (animation.function == "linear" && m_clientCall) {
    ...
    if (animation.tickCount == animation.tick || animation.tickCount == 0) {
      ...
      if (static_cast<int>(animation.halfCount %
                           static_cast<int>(std::ceil(result))) == 0) {
        animation.colorIndex1 += 1;
        animation.colorIndex2 = animation.colorIndex1 + 1;
        animation.halfCount = 0;
    ...
    auto timeFraction =
        static_cast<float>(animation.halfCount) / static_cast<float>(result);
    ...
    rgb[0] =
        animation.colorValue[animation.colorIndex1][0] * (1.0 - timeFraction) +
        (animation.colorValue[animation.colorIndex2][0] * timeFraction);

    rgb[1] =
        animation.colorValue[animation.colorIndex1][1] * (1.0 - timeFraction) +
        (animation.colorValue[animation.colorIndex2][1] * timeFraction);

    rgb[2] =
        animation.colorValue[animation.colorIndex1][2] * (1.0 - timeFraction) +
        (animation.colorValue[animation.colorIndex2][2] * timeFraction);
    std::for_each(animation.leds.begin(), animation.leds.end(), [&](int value) {
      simSetShapeColor(ledsHandle[value - 1], nullptr,
                       sim_colorcomponent_ambient_diffuse, rgb.data());
    });
  }
}
...
void LightControlModule::LoadLedsDescription() {
  ...
  if (not Utils::LoadRosParam(m_nh, "led_size", m_size) or
      not Utils::LoadRosParam(m_nh, "leds", leds,
  ...
  try {
    for (auto i{0}; i < leds.size(); ++i) {
      ...
      led.orientation[0] =
          static_cast<float>(static_cast<double>(sublist["rpy"][0]));
      led.orientation[1] =
          static_cast<float>(static_cast<double>(sublist["rpy"][1]));
      led.orientation[2] =
          static_cast<float>(static_cast<double>(sublist["rpy"][2]));
      m_leds[i] = led;
    }
...
void LightControlModule::CreateAndPlaceLeds() {
  ...
  for (uint i = 0; i < m_leds.size(); ++i) {
    m_ledsHandle[i] =
        simCreatePureShape(SHAPE, FLAGS, m_size.data(), MASS, nullptr);
    if (m_ledsHandle[i] == -1) {
      ROS_WARN(" couldn't load leds description file");
    } else {
      simSetObjectName(m_ledsHandle[i],
                       ("led" + std::to_string(m_leds[i].id)).c_str());
      simSetObjectPosition(m_ledsHandle[i], GetBaseHandle(),
                           m_leds[i].pos.data());
      simSetObjectOrientation(m_ledsHandle[i], GetBaseHandle(),
                              m_leds[i].orientation.data());
